...
export const AgentEditorHeader: React.FC<AgentEditorHeaderProps> = ({
  ...
  const lastSavedAt = useAgentDraftStore.use.useLastSavedAt();
  const isDirty = useAgentDraftStore.use.useIsDirty();
  const activeTab = useAgentUIStore.use.useActiveTab();
  const setActiveTab = useAgentUIStore.use.useSetActiveTab();
  ...
  const savedAtLabel = lastSavedAt
    ? formatUtils.date(lastSavedAt, "YYYY-MM-DD HH:mm:ss")
    : "--:--";
  const statusLabel = isDirty ? "正在自动保存..." : "自动保存已开启";
  const statusDotClassName = isDirty
        ...
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-6 h-16 px-6">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <AgentLogo
              logo={draft.logo}
              name={draft.name}
              className="w-10 h-10 rounded-2xl bg-linear-to-br from-blue-500 to-indigo-500 overflow-hidden"

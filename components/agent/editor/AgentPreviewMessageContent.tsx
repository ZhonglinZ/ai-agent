...
import {
  AgentPreviewComponentBlock,
  AgentPreviewFormBlock,
} from "./AgentPreviewCustomBlocks";
...
const CodeBlock: React.FC<{ raw: string; language?: string }> = ({
  raw,
  language,
}) => {
  return (
...
const createMarkdownComponents = (
  ...
  code: ({ className: codeClassName, children, ...props }) => {
    ...
    const isInlineCode = !codeClassName && !raw.includes("\n");
    ...
    if (language === "component") {
      return (
        <AgentPreviewComponentBlock raw={raw} isStreaming={isStreaming} />
      );
    }

    if (language === "csv" || language === "excel") {
      return (
        <div className="space-y-2">
          <div className="flex items-center justify-between text-[11px] text-gray-500">
            <span>CSV / Excel 预览</span>
            <AgentPreviewCopyButton
              text={raw}
              label="复制 CSV"
              copiedLabel="已复制"
...
export const AgentPreviewMessageContent: React.FC<
  AgentPreviewMessageContentProps

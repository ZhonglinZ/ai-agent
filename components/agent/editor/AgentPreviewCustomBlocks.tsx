...
const renderStreamingPlaceholder = (text: string, className?: string) => (
  <div
    className={cn(
      "rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-500",
      className,
    )}
  >
    {text}
...
const FieldValuePreview: React.FC<{ field: FormFieldConfig }> = ({ field }) => {
  ...
  const sharedClass =
    "w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-gray-600";
...
export const AgentPreviewFormBlock: React.FC<{
  ...
  if (!config || fields.length === 0) {
    ...
    return (
      <div
        className={cn(
          "rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600",
          className,
        )}
      >

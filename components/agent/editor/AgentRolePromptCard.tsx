...
export const AgentRolePromptCard: React.FC<AgentRolePromptCardProps> = ({
  className,
}) => {
  ...
  const handleApplyTemplate = (templateId: string) => {
    const selected = AGENT_PROMPT_TEMPLATES.find(
  ...
  return (
    <section
      className={cn(
        "rounded-2xl border border-gray-200 bg-white p-4 space-y-3",
        className,
      )}
    >
      ...
      <AgentPromptTemplateDrawer
        ...
        onApply={(template: any) => handleApplyTemplate(template.id)}
      />
    </section>
  );
};
